...
export default function HomePage() {
  ...
  useEffect(() => {
    setMeta({
      title: 'SabiLink Marketplace | Browse Services',
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)]">
      <header className="sticky top-0 z-40 border-b border-[var(--color-border)] bg-white/95 backdrop-blur">
        <div className="container-app py-2">
          <div className="flex items-center gap-3">
            <Link to="/" className="inline-flex shrink-0 items-center gap-2" aria-label="SabiLink homepage">
              <img src="/logo.svg" alt="" className="h-9 w-9 object-contain" />
              <span className="text-lg font-extrabold text-slate-900">SabiLink</span>
            </Link>
            ...
            <div className="ml-auto flex items-center gap-1 sm:gap-2">
              <Link
                ...
                className="border border-[var(--color-border)] px-3 py-2 text-[11px] font-bold text-slate-700 hover:bg-slate-100 sm:text-sm"
              >
                {user ? 'Profile' : 'Login'}
              </Link>
              <Link
                to={user ? '/dashboard' : '/register'}
                className="bg-[#0a2e5c] px-3 py-2 text-[11px] font-bold !text-white hover:bg-[#061d3b] sm:text-sm"
              >
                Post a Service
              </Link>
            </div>
      ...
      <div className="container-app py-6">
        {/* New Hero Section */}
        <section className="relative overflow-hidden rounded-none bg-[var(--color-primary-soft)] px-6 py-16 md:px-12 md:py-20 mb-8">
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={onSubmitSearch} className="relative group">
                ...
                <input
                  ...
                  className="w-full h-16 pl-14 pr-32 bg-white rounded-none shadow-sm border border-transparent focus:border-[#0a2e5c] focus:ring-4 focus:ring-blue-50 outline-none text-[15px] font-bold text-slate-700 transition-all"
                />
                <button
                  type="submit"
                  className="absolute right-2 top-2 bottom-2 px-8 bg-[#0a2e5c] text-white font-black rounded-none hover:bg-[#061d3b] transition-colors"
                >
                  Search
                </button>
              </form>
              ...
              <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                <div className="relative">
                  <select
                    ...
                    className="appearance-none h-11 pl-4 pr-10 bg-white border border-slate-100 rounded-none text-[13px] font-black text-slate-600 outline-none cursor-pointer hover:border-[#0a2e5c] transition-all"
                  >
                ...
                <div className="relative flex-1 min-w-[120px] max-w-[180px]">
                  ...
                  <input
                    ...
                    className="w-full h-11 pl-10 pr-4 bg-white border border-slate-100 rounded-none text-[13px] font-black text-slate-700 outline-none focus:border-[#0a2e5c] transition-all"
                  />
                </div>
                ...
                <div className="relative flex-1 min-w-[140px] max-w-[220px]">
                  ...
                  <select
                    ...
                    className="appearance-none w-full h-11 pl-10 pr-4 bg-white border border-slate-100 rounded-none text-[13px] font-black text-slate-600 outline-none cursor-pointer hover:border-[#0a2e5c] transition-all"
                  >
                    <option value="">
                      {categoriesLoading
                        ? 'Loading...'
                ...
                <div className="flex items-center gap-3 py-2">
                  <span className="text-[12px] font-black text-slate-500 uppercase tracking-wider">Verified</span>
                  <button
                    type="button"
                    onClick={() => updateParams({ verified: !verified ? '1' : '' })}
                    className={`relative w-12 h-6 rounded-none transition-colors ${verified ? 'bg-[#0a2e5c]' : 'bg-slate-300'}`}
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-none transition-all ${verified ? 'left-7' : 'left-1'}`} />
                  </button>
                </div>
              </div>
            </div>
        ...
        <section>
          ...
          {pagination.lastPage > 1 ? (
            <div className="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-none border border-slate-100 bg-white p-6 shadow-sm">
